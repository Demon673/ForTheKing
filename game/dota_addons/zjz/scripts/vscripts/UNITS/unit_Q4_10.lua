...
local brain_OnAttacked = function( u_self, u_attacker, f_damage )
	if ((u_self:GetHealth() / u_self:GetMaxHealth() < 0.5) and (RandomInt(0, 99) < 25)) then
...
local skill_01_passive = function( u_self )
	...
	if ((f_health_percentage >= 0.5) and (BuffManager:GetCount( u_self, "modifier_Q4_10_attack_speed" ) == 0)) then
		BuffManager:Add( u_self, "modifier_Q4_10_attack_speed", particle_function, false )
	end

	if ((f_health_percentage < 0.5) and (BuffManager:GetCount( u_self, "modifier_Q4_10_attack_speed" ) == 1)) then
		BuffManager:Remove( u_self, "modifier_Q4_10_attack_speed" )
	end
