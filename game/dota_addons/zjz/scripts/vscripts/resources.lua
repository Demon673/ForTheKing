...
--资源系统刷新
	function resources:OnThink() 
		print("run")
    	for i = 0,8,1 do
        	local p = PlayerResource:GetPlayer(0) --按ID索引玩家
            		if p ~= nil and i~=4 then
            			--self:updateGold(p)
            			--PlayerResource:SetGold(i,123, true) 
            			print(i)
            		end
        end
        return 1
	...
	--function resources:updateGold(p)
		--local player = p
		--local player_id = player:GetPlayerID() 
		--local money = player.Gold
		--PlayerResource:SetGold(0,123, true) 
		--print("money")
	--end--]]



	--[[function resources:AddLumber(keys)
